...
let
  ...
  nn = cfg.notifications.ntfy; # New ntfy notification options
  ...
  smartdNotify = pkgs.writeScript "smartd-notify.sh" ''
    ...
    ${optionalString nn.enable ''
      ...
      $SMARTD_FULLMESSAGE
      EOF
      } | ${pkgs.curl}/bin/curl -H "Priority: urgent" -H "Title: SMART" -d @- https://ntfy.alexanderdinges.de/smart &
    ''}
  '';

  notifyOpts = optionalString (nm.enable || nw.enable || nx.enable || nn.enable)
      ...
      notifications = {
        ...
        ntfy = {
          enable = mkOption {
            default = false;
            type = types.bool;
            description = lib.mdDoc "Whether to send notifications via ntfy.";
          };
        };
      };
